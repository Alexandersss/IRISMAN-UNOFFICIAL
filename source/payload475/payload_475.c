...
#define SYSCALL_BASE                    0x8000000000363BE0ULL // 4.75 CEX WOW!
#define NEW_POKE_SYSCALL                813
#define NEW_POKE_SYSCALL_ADDR			0x80000000002A1480ULL // SYSCALL (813) done
...
static inline void remove_lv2_memcpy()
{
    ...
    /* restore syscall */
    //remove_new_poke();
// Found @ 0x2A1480 This is per FW, syscall_rmdir // SC813
...
*/
    ...
    pokeq(0x80000000007EF220ULL, 0ULL);
			 ...
			 // patches by deank ( These patches have been fixed M@tsumot0 ) 4.75 these patches are replaced by SYS36 PATCH
    ...
    _poke32(0x56600, 0x60000000);             // 0x419D00C02F840004ULL
    PATCH_JUMP(0x56608, 0x566A0);             // 0xE86298687C8407B4ULL / 0x3BE000004BFFFEC0ULL
    _poke32(0x5A6E4,  0x60000000);            // fix 80010009 error --> 0x419E00ACE8610188ULL
    _poke32(0x5A6F8,  0x60000000);            // fix 80010019 error --> 0x419E00AC38610070ULL
    _poke(0x5658C,  0x63FF003D60000000); // fix 8001003D error  "ori     %r31, %r31, 0x3D\n nop\n" done --> 0x63FF003D60000000ULL
    _poke32(0x56654,  0x3BE00000);            // fix 8001003E error -- 3.55 ok in 0x055F64 "li      %r31, 0"  done --> 0x3BE000004BFFFF0CULL

    //Fix 0x8001003C error (incorrect version in sys_load_param) - It is present in the new game updates **/
    _poke(0x267150, 0x386000007C6307B4); // 0x8003026C3D600047ULL
    ...
    */
    ...
    PATCH_JUMP(0x29765C, (PAYLOAD_OFFSET+0x30)); // patch openhook ( instead "mr        r29, r3" ) <-- Alexander .done
...
static int lv2_unpatch_storage_475(void)
{
    ...
    pokeq(0x80000000002ECAB0ULL, save_lv2_storage_patch); //done
