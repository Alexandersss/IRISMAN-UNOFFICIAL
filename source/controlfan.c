...
bool test_controlfan_compatibility()
{
    if(firmware == 0x341C || firmware == 0x355C || firmware == 0x355D || firmware == 0x355E ||
       ...
       firmware == 0x460C || firmware == 0x460D || firmware == 0x460E || firmware == 0x465C || firmware == 0x465D || firmware == 0x466C ||
       firmware == 0x466D || firmware == 0x470C || firmware == 0x470D || firmware == 0x475C || firmware == 0x475D || firmware == 0x476C || firmware == 0x476D
      )
        return true;
    else
        return false;
...
void load_controlfan_config()
{
    ...
    if(file_exists(temp_buffer + 2048) == false)
        sprintf(temp_buffer + 2048, "/dev_hdd0/game/IRISMAN00/config/fancontrol.dat");
