...
#define NEW_POKE_SYSCALL_ADDR           0x80000000001AE034ULL  // SYSCALL (813) done

#define PAYLOAD_OFFSET                  0x3d90
#define PERMS_OFFSET                    0x3560

#define PAYLOAD_UMOUNT_OFFSET           (0x3d90+0x400)
#define UMOUNT_SYSCALL_OFFSET           (0x1AC74C + 0x8) //  SYSCALL (838) + 8 -> peek(SYSCALL_BASE + 838*8)
...
void load_payload_460deh(int mode)
{
    ...
    */
			...
			pokeq(0x80000000002373FCULL, 0x38600000F8690000ULL ); // fix 0x8001002B / 80010017 errors (2015-01-03)  - 460 DEH ok!
...
static int lv2_unpatch_storage_460deh(void)
{
    ...
    regs_i.reg3 = 0x18df14; regs_i.reg4 = save_lv1_storage_patches[0];
    sys8_lv1_syscall(&regs_i, &regs_o);

    regs_i.reg3 = 0x18f55c; regs_i.reg4 = save_lv1_storage_patches[1];
    sys8_lv1_syscall(&regs_i, &regs_o);

    regs_i.reg3 = 0x18f5d4; regs_i.reg4 = save_lv1_storage_patches[2];
    sys8_lv1_syscall(&regs_i, &regs_o);

    regs_i.reg3 = 0x18f5dc; regs_i.reg4 = save_lv1_storage_patches[3];
    sys8_lv1_syscall(&regs_i, &regs_o);
