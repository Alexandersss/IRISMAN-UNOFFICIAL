 ...
 */
// Alexander's port 3.55 DECR/DEH
...
void load_payload_355deh(int mode)
{
    ...
    pokeq(0x80000000007EF220ULL, 0ULL);
			 ...
			 // patches by deank ( These patches have been fixed Alexander ) 3.55 DECR these patches are replaced by SYS36 PATCH
	/*
			...
			pokeq(0x8000000000059C58ULL, 0x63FF003D60000000ULL ); // fix 8001003D error  Original: 0x63FF003D419EFFD4ULL
			pokeq(0x8000000000059D1CULL, 0x3FE080013BE00000ULL ); // fix 8001003E error  Original: 0x3FE0800163FF003EULL
    ...
    */
    ...
    _poke32(0x59CCC, 0x60000000);             // 0x419D00C02F840004ULL
    PATCH_JUMP(0x59CD4, 0x59D6C);             // 0x409C0048E8629828ULL
    _poke32(0x7F6D4,  0x60000000);            // fix 80010009 error --> 0x419E0020E8610138ULL
    _poke32(0x7F6E8,  0x60000000);            // fix 80010019 error --> 0x419E00208061007CULL
    _poke(0x59C58,  0x63FF003D60000000); 		// fix 8001003D error  "ori     %r31, %r31, 0x3D\n nop\n" done
    _poke32(0x59D20,  0x3BE00000);            // fix 8001003E error -- 3.55DEH ok in 0x59D20 "ori       r31, r31" fixed by aldostools

    PATCH_JUMP(0x59D24, 0x59C30);

    //Fix 0x8001003C error (incorrect version in sys_load_param) - It is present in the new game updates **/
    _poke(0x291798, 0x386000007C6307B4); // 0x8003026C3D600035ULL # binary correct Alexander
    _poke32(0x291798 + 8, 0x4E800020);  // 0x8003026C3D600035ULL # binary correct Alexander
